...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-[#0A192F]/90 backdrop-blur-xl border-b border-primary/20">
      <div className="container mx-auto px-2 md:px-4 h-20 flex items-center justify-between gap-2 md:gap-4">
        {/* Logo Section */}
        ...
        {/* Navigation - Adjusted for mobile visibility */}
        <nav className="flex-1 flex justify-center items-center gap-1.5 md:gap-10 py-2">
          {[
            { name: 'হোম', link: '/' },
            ...
            <Link 
              key={item.name} 
              href={item.link} 
              className="text-[9px] min-[400px]:text-[10px] md:text-sm font-bold text-white/70 hover:text-primary transition-all whitespace-nowrap px-0.5 md:px-2 relative group"
            >
              {item.name}
        ...
        <div className="shrink-0">
          <Button 
            ...
            className="bg-primary hover:bg-primary/80 text-black font-black text-[9px] md:text-xs rounded-xl px-2 md:px-7 h-9 md:h-10 shadow-[0_0_15px_rgba(0,255,255,0.4)] hover:shadow-[0_0_25px_rgba(0,255,255,0.6)] transition-all border border-white/20"
          >
            <QrCode className="w-3 h-3 md:w-4 md:h-4 mr-1" /> <span className="hidden min-[380px]:inline">আইডি যাচাই</span><span className="min-[380px]:hidden">যাচাই</span>
          </Button>
        </div>
      </div>
      {/* Bottom Glow Line */}
